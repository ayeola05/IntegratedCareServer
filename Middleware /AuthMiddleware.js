...
const protectPractitioner = asyncHandler(async (req, res, next) => {
  let token;
  ...
  if (
    ...
    try {
      ...
      req.user = await Practitioner.findById(decoded.id).select("-password");
      next();
    } catch (error) {
      console.error(error);
      res.status(401);
...
const isPractitioner = (req, res, next) => {
  if (req.user && req.user.registrationNumber) {
    ...
    res.status(401);
    throw new Error("Not authorized as a practitioner");
  }
};
